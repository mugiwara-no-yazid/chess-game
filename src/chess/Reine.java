...
public class Reine extends Piece {
    ...
    @Override
     public void bouger(Piece piece, int startX, int startY,int posx,int posy)
     {
      ...
      private boolean MouvPion(Reine reine, int startX, int startY, int endX, int endY) {
        int direction = reine.estBlanc == true ? 1 : -1;
        ...
        if (startX == (reine.estBlanc == true ? 1 : 6) && endX == startX + 2 * direction && endY == startY) {
            return true;
        }
